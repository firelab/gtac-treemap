...
# path to LF slow loss layers
slowloss_dir <- paste0(home_dir, "03_Outputs/02_Landfire_Slow_Loss/01_Rasters/")
...
slowloss_filename <- paste0(start_year, "_", end_year, "_UT_Uintas_subset_Landfire_SlowLoss.tif")


#set path to save evaluation data
eval_dir <- paste0(home_dir, "03_Outputs/02_Landfire_Slow_Loss/02_Evaluation/")
...
# project to crs
slowloss_bin <- project(slowloss_bin, crs, method = "near")
...
# currently exporting: mortality status class
...
if (ncol(cm_t_classes == 1)){
  names(cm_t_classes) <- "value"
}
