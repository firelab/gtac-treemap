#for (z in c(8,7,10)){
  ...
  # Check on this - Plot ID
  plot.df$ID<- row.names(plot.df)
  
  #Create X Tables for imputation ---
  x.df<- plot.df[,c(3,5:21)]
  ...
  # Updated
  y.df <- data.frame(plot.df[,c(15:17)])
  ...
  print(paste0(length(which_tiles), " tiles to process"))
  ...
  lookup <- left_join(id_list, plot.df, by = c("PLOTID" = "ID")) %>%
    select(PLOTID, plt_cn, all_of(eval_vars)) %>%
  ...
  #evg_reclass$EVT_GP_remap<- as.factor(evg_reclass$EVT_GP_remap)
  #lookup<- left_join(lookup, evg_reclass, by = c("evt_gp" = "EVT_GP_remap"))
  
  
  # overwrite the original evt gp with the Remap
  #lookup$evt_gp<- lookup$EVT_GP
  #lookup<- lookup[,-ncol(lookup)]
  ...
  # Remove all objects from environment except m
