...
for(v in 1:length(eval_vars)){
  ...
  var_in <- eval_vars[v]
  # Because variable names are now "evc" rather than "canopy_cover", change variable names for plotting
  var_in_name <- eval_vars_full_names[v]
  

  cat("\n\n\\pagebreak\n")
  print(glue("**Variable: {var_in_name}**
  ...
  rat_z<- plot.df
    # add binary disturbance if necessary
  if(var_in == "binary_disturbance"){
   rat_z$binary_disturbance<- rat_z$disturb_code
   rat_z$binary_disturbance[rat_z$binary_disturbance == 2]<-1
  }
  # Rename ID to PLOTID
  ...
  barchart1 <- freq_t1 %>%
    ...
    ggtitle(glue("Frequency of classes
            Zone: z{zone_num} ; Attribute: {var_in_name}"))
  ...
  # get producer's accuracy for each class - precision
    ...
    # Need an additional fix for binary disturbance because confusion matrix only outputs one column for it
  if(var_in == "binary_disturbance"){
    pa <- cm_classes %>%
      ...
      # add extra row to address binary disturbance being mishandled by confusion matrix
      rbind(round(cms[[1]][2,2] / cms[[1]][2,3],round_dig)) %>%
  ...
  if(var_in == "binary_disturbance"){
    ...
    # replace NA with character
    mutate(value = as.character(value)) %>%
    # add extra row to address issue with binary disturbance
    rbind(round(cms[[1]][2,2] / cms[[1]][3,2],round_dig))  %>%
    replace_na(list(value = "NA"))} else{
    #
    ua <- cm_classes %>%
    filter(metric == "Recall") %>% 
    mutate(value = round(value, round_dig))  %>%
  ...
  cm_display$Pred <- row.names(cm_display)
  row.names(cm_display) <- NULL
  
  # bind row name column back with df
  cm_display <- cbind(Imputed = cm_display$Pred, cm_display[1:ncol(cm_display)-1])
  ...
  table_cm <- flextable(cm_display) %>%
    ...
    add_header_row(values = c("", "Target"), colwidths = c(1, ncol(cm_display)-1)) %>%
