...
# set factors for dividing cores between tiles and rows
#cores_tiles <- 10
#cores_rows <- 2
...
#     future::tweak(
#       future::multisession,
#       workers = floor(ncores/cores_tiles)),
#     future::tweak(
#       future::multisession,
#       workers = floor(ncores/cores_rows))
#   )
# )
...
for(j in 1:length(tiles_run)) {
  ...
  # get coordinates of each cell as layer
  lon <- terra::init(ras, "x")
  names(lon) <- "POINT_X"
  lat <- terra::init(ras, "y")
  names(lat) <- "POINT_Y"

  # combine coords with other input layers
  ras <- c(ras, lon, lat)
  ...
  # get col names from raster
  colnames <- names(ras)
  
  # convert raster to matrix
  mat <- as.matrix(ras)
  
  # wrap terra object so it can be used in parallel
  #ras <- terra::wrap(ras)
  ...
  rows_out <-
    ...
    #imap(function(fn, i, ...) {
      ...
      # prep data frame from matrix for single row
      # matrix isn't organized by rows any more; subset ncol_r values at a time
      # dat_ext  <- data.frame(
      #                             mat[(ncol_r*(i-1)+1):(ncol_r*i),] )

      # impute on row of data - input is named row of data + yai
      # make sure factor levels match, make sure non-appearing evgs appear
  ...
  # don't write out more data than we need to, even if they're NAs
  ...
  # export
  writeRaster(tile_out, 
              # glue doesn't work for file names within future loop
              #glue::glue('{output_dir}raster/tiles/{output_name}_testRows_{testrow1}_{testrow2}_maxpx{max_px}_nT{ntiles}_tile{i}.tif'),
              paste0(output_dir,'raster/tiles/',output_name,'NEWIMPTest_testRows_',row1,'_',row2,'_maxpx',max_px,'_nT',ntiles,'_tile',j,'.tif'),
              overwrite = TRUE)
