...
# Load All X-tables, and bind as one
# ----------------------------------------------------------#
xtable <- list.files(path=xtable_dir, full.names = T) %>% 
  lapply(read.csv) %>% 
  bind_rows

# Remove duplicate observations in the X-tables
xtable<- xtable[,-1] # remove "Zone" column
xtable<- xtable[!duplicated(xtable), ]
...
xtable<- xtable[xtable$EVT_GP %in% evt_gp_remap_table$EVT_GP,]
