...
#### General: 
- Control script to run multiple zones in a row-- for target data prep, or imputation, assembly, etc. 
...
- update to refer to base yaImpute installation, after PR is merged with Lila's fork for calculating OOB error 
...
#### Target data:
- Script to download DayMet climate data
- Script to process DayMet climate data
- Script to process veg and disturbance layers to make them appropriate for 2020 vs 2022
...
- ~~Perform data frame processing with Tidyverse~~
	...
	- Are there instances where data.frame is better? 

#### Assembly
- Calculate FIA derivatives for final model assembly - Rachel/Lila/Abhi
	...
	- Visit FIA db for existing calculations-- identify other vars we might like to report
	- Code to calculate these vars -- either using SQL in R, or table-based dplyr calcs in R
	- Determine if we need to download additional tables from FIA db to calculate additional vars
	

#### Validation
- Make plotting and eval report generation more modular 
  - Functions for plotting
  - RMD input more flexible / RMD generation script more flexible? 
...
#### Target data prep
- ~~Script to download Landfire data from Landfire site~~ 
	- ~~Disturbance~~
	- ~~Topo~~
	- ~~EVC~~
	- ~~EVH~~
