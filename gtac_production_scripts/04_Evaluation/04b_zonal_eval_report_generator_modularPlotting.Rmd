...
for(v in 1:length(eval_vars_cat)){
  ...
  if(var_in %in% c("evc","evh","disturb_code_bin") & eval_type == "TargetLayerComparison") {
  ...
  message(paste0("KL divergence between X-table and Target: ", round(suppressMessages(philentropy::KL(x= rbind(freq_t2$frequency[freq_t2$dataset == "ref"], freq_t2$frequency[freq_t2$dataset == "aRAT"]), unit="log2")),4)))
  message(paste0("KL divergence between X-table and Imputation: ", round(suppressMessages(philentropy::KL(x= rbind(freq_t2$frequency[freq_t2$dataset == "pred"], freq_t2$frequency[freq_t2$dataset == "aRAT"]), unit="log2")),4)))
