...
subtitle: "`r params$raster_name` ; `r params$ref_type`"
output:
...
"Reference" data source: `r params$ref_type`
...
for(v in 1:length(var_names)){
  ...
  freq_norm$aRAT <- rat_freq$Freq_norm
  ...
  barchart1 <- freq_t1 %>%
    ...
    scale_fill_manual(name = "Dataset", labels = c("Imputed", "Observed" ),values = c("aquamarine3", "chartreuse4"))+
    theme_bw() +
    ggtitle(glue("Frequency of classes for imputed vs. observed data
  ...
  # Bar plot: Noramlized frequency, observed vs. predicted vs. for each class 
  # ---------------------------------------------------------#
  ...
  barchart2 <- freq_t2 %>%
    ggplot() +
    geom_col(aes(x = class, y = frequency, fill = factor(dataset)), position = "dodge") +
    scale_fill_manual(name = "Dataset", labels = c("Reference", "Imputed","Observed"),values = c("springgreen4", "aquamarine3", "chartreuse4"))+
    theme_bw() +
    ggtitle(glue("Normalized frequency of classes for reference vs. imputed vs. observed data
            Zone: z{zone_num} ; Attribute: {var_in}"))
