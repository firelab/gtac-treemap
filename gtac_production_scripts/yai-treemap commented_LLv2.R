...
# Load points from export from John
...
fia_pts_xy %<>% cbind(data.frame(fia_pts)) %>%
  ...
  #dplyr::select(CN_xy, PREV_PLT_C, x, y, PLOT, LON_ACTUAL, LAT_ACTUAL) %>%
  dplyr::select(CN_xy, x, y, PLOT, LON_ACTUAL, LAT_ACTUAL) %>%
  dplyr::arrange(PLOT) %>%
  dplyr::distinct()
...
merge.df<- left_join( plot.df , fia_pts_xy, by = c("CN" = "CN_xy"))
dim(merge.df)
