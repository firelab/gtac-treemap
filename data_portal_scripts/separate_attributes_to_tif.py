...
# Metadata strings (globally declared to synchronize text changes between create_xml_metadata() + create_html_metadata() functions)
pub_appended_text = ' (source data)'
abst_remove_text = ' (the GeoTIFF included in this data publication)'
...
cols = [('FORTYPCD', 'Algorithm Forest Type Name', gdal.GDT_UInt16), ('FLDTYPCD', 'Field Forest Type Name', gdal.GDT_UInt16),
        ...
        ('STANDHT', 'Height of Dominant Trees (feet)', gdal.GDT_UInt16), ('ALSTK', 'All Live Tree Stocking %', gdal.GDT_Float32), 
        ('GSSTK', 'Growing-stock stocking %', gdal.GDT_Float32), ('QMD_RMRS', 'Stand Quadratic Mean Diameter', gdal.GDT_Float32),
        ('SDIPCT_RMR', 'Stand Density Index (percent of maximum)', gdal.GDT_Float32), ('TPA_LIVE', 'Live Trees Per Acre', gdal.GDT_Float32),
        ('TPA_DEAD', 'Dead Trees Per Acre', gdal.GDT_Float32), ('VOLCFNET_L', 'Live Volume SawLog (board-ft/acre)', gdal.GDT_Float32), 
        ('VOLCFNET_D', 'Standing Dead Volume (ft^3/acre)', gdal.GDT_Float32), ('VOLBFNET_L', 'Live Volume (ft^3/acre)', gdal.GDT_Float32),
        ('DRYBIO_L', 'Above Ground Dry Live Tree Biomass (tons/acre)', gdal.GDT_Float32), ('DRYBIO_D', 'Above Ground Dry Standing Dead Tree Biomass (tons/acre)', gdal.GDT_Float32),
        ('CARBON_L', 'Live Carbon Above Ground (tons/acre)', gdal.GDT_Float32), ('CARBON_D', 'Standing Dead Carbon (tons/acre)', gdal.GDT_Float32), 
        ('CARBON_DWN', 'Down Dead Carbon (tons/acre)', gdal.GDT_Float32)]
...
def attributeToImage(columnName, columnFullName, gdal_dtype):
    ...
    Args: 
        ...
        columnFullName (str): Full name of attribute to be processed (e.g. Standing Dead Carbon)
...
def create_xml_metadata(col_name):
    '''
    Creates xml metadata file based on an original xml file (scripted to work on TreeMap2016, should work on other versions if metadata format is the same, small tweaks may be necessary.)
    ...
    publisher.text += pub_appended_text

    # Abstract
    abstract = root.find('.//abstract')

        # Remove irrelevant text from the abstract and append information regarding the specific attribute
    abstract.text = abstract.text.replace(abst_remove_text, '')
    abstract.text += abst_appended_text.format(year=year, col_name=col_name)

    # Entity and Attribute Overview
    eaover = root.find('.//eaover')
        # Replace text from eaoverview with new text
    eaover_important_info_formatted = eaover_important_info.format(year=year, col_name=col_name)
    eaover_tifdescription_formatted = eaover_tifdescription.format(year=year, col_name=col_name, col_description=col_descriptions[col_name])
    eaover.text = f'''
    Below is a description of the file included in this publication.

    IMPORTANT INFORMATION

    {eaover_important_info_formatted}
    

    DATA FILE DESCRIPTIONS

    (1) TreeMap{year}_{col_name}.tif:
    {eaover_tifdescription_formatted}
    '''
    
    # Process Description
    procdesc = root.find('.//procdesc')
        # Insert text before citations
    insert_text_before_xml(procdesc, 'For complete details see', procdesc_inserted_text.format(col_name=col_name))

    # Standard Order Process
    stdorder = root.find('.//stdorder')
        # Remove extra digital forms and update the file compression text + online link in the remaining form
    remove_extra_digital_forms_xml(stdorder)
    stdorder.find('.//filedec').text = file_compression_text
    stdorder.find('.//networkr').text = data_gateway_link
...
def create_html_metadata(col_name):
    ...
    if publisher:
        ...
        new_publisher_tag = soup.new_string(' Forest Service Research Data Archive' + pub_appended_text)
    ...
    if abstract_text:
        ...
        # Go three levels deeper to the third 'dd' tag
        inner_dd = abstract_text.find(lambda tag: has_specific_text(tag, 'dd', 'We matched forest plot data from Forest Inventory and Analysis (FIA) to a 30x30 meter (m) grid'))
        inner_dd_2 = inner_dd.find(lambda tag: has_specific_text(tag, 'dd', 'We matched forest plot data from Forest Inventory and Analysis (FIA) to a 30x30 meter (m) grid'))
        
        # Delete specified text from abstract
        replace_text_html(inner_dd_2, abst_remove_text, '')
        # Add new text
        abst_appended_text_formatted = abst_appended_text.format(year=year, col_name=col_name)
        abstract_newtext = f'''<br/><br/>
        {abst_appended_text_formatted}
        <br/>
        '''
        
        # Add modified content to the parsed HTML
        new_content_soup = BeautifulSoup(abstract_newtext, 'html.parser')
        inner_dd_2.append(BeautifulSoup('<br/>', 'html.parser'))
        for element in new_content_soup.contents:
            if isinstance(element, NavigableString):
                inner_dd_2.append(BeautifulSoup(str(element), 'html.parser'))
            else:
                inner_dd_2.append(element)
    ...
    procdesc = soup.find(lambda tag: tag.name == 'br' and tag.find_next_sibling(string=True) is not None and 'For complete details see Riley et al. (2016)' in tag.find_next_sibling(string=True))
    
    if procdesc:
        # Create a new string with the new paragraph
        new_paragraph = soup.new_string(procdesc_inserted_text.format(col_name=col_name))
        ...
        procdesc.insert_after(new_paragraph, new_br_tag1, new_br_tag2)
        ...
        <dl>
         ...
         <dd>
          <dl>
           ...
           <dd>GeoTIFF file</dd>
           <dt>
            <i>File_Decompression_Technique: </i>{file_compression_text}</dt>
          </dl>
         </dd>
         <dt>
               ...
               <dd>
                <dl>
                 <dt>
                  ...
                  <a
                   TARGET="viewer"
                   href="{data_gateway_link}">{data_gateway_link}</a>
                 </dt>
                </dl>
               </dd>
              </dl>
...
def prompt_user():
    ...
    print("If filepaths and chunk size are correct press 'enter', otherwise press 'q' to quit and please update filepaths or chunk size in script:")
    key = msvcrt.getch()
    if key.lower() == b'q':
        quit()
