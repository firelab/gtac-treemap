...
curzone <- "z64"
curfolder <- paste("G:\\TreeMap2016\\outputs\\fixpptsc\\", curzone, sep="")

xtableall <- read.table("G:\\TreeMap2016\\x_table\\X_table_all_singlecondition.txt", header=T, sep=",")

evglist <- read.table(paste("G:\\TreeMap2016\\working_KLR\\EVG_remap\\", curzone, "_EVG_remap.txt", sep=""), sep=":")

xtable <- xtableall[(xtableall$EVT_GP %in% evglist[,1]),]
...
# recode to two disturbances
recodevec <- which(dcs[,3]==2)
dcs[recodevec,3] <- 1
recodevec <- which(dcs[,4]==2)
dcs[recodevec,4] <- 1
dcvec <- c(0,1)
# make confusion matrix
dcmat <- matrix(data=0, nrow=2, ncol=2, byrow=FALSE)
for (j in 1:length(dcvec))
{
  ...
  {
    tempvec2 <- which(tempmat[,4]==dcvec[k])
    tempmat2 <- tempmat[tempvec2,]
    if (dim(tempmat2)[[1]]==1) { dcmat[j,k] <- tempmat[tempvec2,2] } # assign count to matrix
    if (dim(tempmat2)[[1]]>1) { dcmat[j,k] <- sum(tempmat2$Count) }
  }
}
...
# bar chart
dcplottotals <- c(sum(dcmat[,1]), sum(dcmat[,2]))
sum(dcplottotals)
dcpolytotals <- c(sum(dcmat[1,]), sum(dcmat[2,]))
sum(dcpolytotals)
...
dccount2 <- c(dccount[1], dccount[2]+dccount[3])
plot(dccount2, accuracy4, xlab="Count of plots in disturbance code class", ylab="Accuracy")
dcacc2 <- cbind(dcvec, dccount2, accuracy4)
...
dyvec <- sort(unique(c(dys$disturb_y1, dys$disturb_y2)))
# make confusion matrix
