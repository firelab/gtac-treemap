...
gm_auth_configure(key = "111394764600-cko2lhrkspargbdn3liquvmadjbubgc0.apps.googleusercontent.com", secret = "KZGW_I9kqJikgJN3T9SrZbGs")

subj.str <- paste("Zone ", cur.zone, "Starting...")
text.str <- subj.str
...
setwd("G:\\Workspace\\treemap\\treemap2014_rasters2016")

allplot.2016 <- read.table("x_table_TreeMap2016_final.csv", header = TRUE, sep = ",")


###subset to only single condition plots
...
{  
  ...
  extract.ppt.good <- extract(ppt.good.raster, sp.currow)  
  extract.currow <- data.frame(extract.currow)
  
  extract.currow$"PPTI" <- extract.ppt.good
...
fout <- paste(cur.zone, "_2014-xtable-2016-raster-fixppt.tif", sep="")
